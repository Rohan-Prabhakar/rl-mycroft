...
th.set_num_interop_threads(1)  # Critical for Python 3.13 Windows
    ...
    def train(
        ...
        if callback is None:
            callback = PortfolioEvalCallback(
                eval_freq=5000,   # Less frequent evals during speed test
                log_dir=log_dir,
                best_model_save_path="./models/best",
                verbose=0         # Silence eval logs
            )
        else:
        ...
        # CRITICAL: Disable tqdm progress bar (massive Windows slowdown)
        self.model.learn(
            total_timesteps=self.total_timesteps,
            callback=callback,
            tb_log_name=tb_log_name,
            reset_num_timesteps=True,
            progress_bar=False    # Removes Python-level tqdm overhead
        )
